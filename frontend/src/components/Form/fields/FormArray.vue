<template>
  <div class="mb-4">
    <label v-if="item.label || item.title" :for="`field-${item.name}`" class="block font-medium text-sm text-slate-700 mb-2">
      {{ item.label || item.title }}
      <span v-if="item.required" class="text-red-500">*</span>
    </label>

    <div class="space-y-3" :id="`field-${item.name}`">
      <div
      ...
      >
        <div class="flex gap-2 items-start">
          <div class="flex-1">
            ...
            <div v-if="isObjectMode()" class="grid gap-3" :style="{ gridTemplateColumns: `repeat(${item.fields?.length || 1}, minmax(0, 1fr))` }">
              <div v-for="field in item.fields" :key="field.name">
                <label v-if="field.label" class="block text-xs font-semibold text-slate-600 mb-1 tracking-wide uppercase">{{ field.label }}</label>
                <input
                  type="text"
    ...
    <p v-if="item.help" class="text-xs text-slate-500 mt-1.5">{{ item.help }}</p>
  </div>
</template>
