<template>
  <div class="mb-4 relative" v-bind="$attrs">
    <label v-if="item?.label || item?.title || label" :for="`field-${item?.name || name}`"
      class="block font-medium text-sm text-slate-700 mb-2">
      {{ item?.label || item?.title || label }}
      <span v-if="item?.required || required" class="text-red-500">*</span>
    </label>

    <div class="relative">
      <!-- Search Input -->
      <div class="relative">
        <input :id="`field-${item?.name || name}`" v-model="searchText" type="text"
          ...
          @focus="onFocus" @blur="onBlur" @input="filterOptions">
        <svg xmlns="http://www.w3.org/2000/svg"
          class="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" viewBox="0 0 20 20"
          fill="currentColor">
      ...
      <div v-if="isOpen && filteredOptions.length > 0"
        ...
        <button v-for="opt in filteredOptions" :key="opt.value" type="button"
          ...
          @click="selectOption(opt)" @mousedown.prevent="selectOption(opt)">
          <div class="flex-1 min-w-0">
            <div class="font-medium text-slate-800 truncate hover:text-primary transition-colors">
              {{ opt.label }}
            </div>
          </div>
          <div v-if="opt.description"
            class="text-xs text-primary-dark bg-primary/10 px-2 py-1 rounded-md shrink-0 whitespace-nowrap">
            {{ opt.description }}
          </div>
        </button>
      </div>

      <div v-if="isOpen && searchText && filteredOptions.length === 0"
    ...
    <p v-if="item?.help || help" class="text-xs text-slate-500 mt-1.5">
      {{ item?.help || help }}
    </p>
  </div>
</template>
